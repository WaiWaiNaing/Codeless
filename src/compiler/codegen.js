...
import { createValidator } from '${relRuntime}/validator.js';
import { authMiddleware, signToken } from '${relRuntime}/auth.js';
import { createSugar } from '${relRuntime}/sugar.js';
...
`;
  ...
  for (const route of ast.routeLines) {
    ...
    server += `    const msg = err instanceof Error ? err.message : String(err);\n`;
    server += `    res.status(status).json({ error: msg, message: msg });\n`;
    server += `  }\n`;
    server += `});\n\n`;
  }

  server += `const port = parseInt(process.env.PORT || '3000', 10);\n`;
  server += `const host = process.env.HOST || '0.0.0.0';\n`;
  server += `app.listen(port, host, () => {\n`;
  server += `  console.log('Codeless v4 server on http://' + host + ':' + port);\n`;
  server += `  if (host === '0.0.0.0') console.log('  Postman / other apps: http://localhost:' + port + ' or http://127.0.0.1:' + port);\n`;
  server += `});\n`;
